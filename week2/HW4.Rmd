...
```{r pred values}
set.seed(25)
daily_bike_weather$dayofweek <- weekdays(daily_bike_weather$ymd)
daily_bike_weather$prevcount <- lag(daily_bike_weather$count)

trainset <- sample_n(daily_bike_weather, .8*nrow(daily_bike_weather))
testset <- setdiff(daily_bike_weather, trainset)

model <- lm(formula = count ~ tmin*prcp+snow+tmax*tmin+dayofweek*prcp+prevcount, data = trainset)
summary(model)
testset$y_hat <- predict(model, newdata = testset)
trainset$y_hat <- predict(model, newdata = trainset)
