...
select(trips, starttime) %>% filter(!is.na(starttime)) %>% mutate(hours = hour(starttime), day = as.Date(starttime)) %>% group_by(day, hours) %>% count() %>% ungroup() %>% group_by(hours) %>% summarize(mean(n))
