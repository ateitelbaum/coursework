...
group_by(ratings, user_id) %>% summarize(numRatings = n()) %>% 
  filter(numRatings < quantile(numRatings, .9)) %>%
...
movie_rank <- group_by(ratings, movie_id) %>% summarize(count = n()) %>% arrange(desc(count)) %>% mutate(rank = row_number())

percentiles <- left_join(ratings, movie_rank) %>% group_by(user_id) %>% summarise(max = max(rank), `90perc` = quantile(rank,.9))
